...
const shopReducer = (state = INITIAL_STATE, action) =>{
    switch(action.type){
        case actionTypes.ADD_TO_CART:
            ...
            const inCart = state.cart.find(item.id === action.payload.id ? true : false)
            return{
                ...state,
                cart: inCart ? state.cart.map(item => action.payload.id ? {...item, qty: item.qty + 1} : item) : [...state.cart,{...item, qty: 1}]
            }
